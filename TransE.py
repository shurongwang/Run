...
class Model(torch.nn.Module):
	...
	def init_traditional_2(self, emb_n):
	...
	def clean(self):
		...
		self.rel_emb.weight.data = F.normalize(self.rel_emb.weight.data, dim = 1)
...
@torch.no_grad()
def eval(data, colo = "green", batch_s = 50):
	model.eval()
...
@torch.no_grad()
def eval_with_filter(data, colo = "green", batch_s = 50):
	model.eval()
...
if st:
	torch.save(model.ent_emb.weight.data, "ent_emb.pth")
	torch.save(model.rel_emb.weight.data, "rel_emb.pth")
